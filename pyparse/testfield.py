...
for line in rdr:
	if not firstpass == 0:	
		if not path.isfile('./data/int_each/'+line[2] +'/'+ line[4]+'_TimeTable.csv'):
			if path.isfile('./data/int_each/'+line[2] +'/'+'부산서부_TimeTable.csv'):
				copylist = []
				firstTemp = 0
				f2 = open('./data/int_each/'+line[2] +'/'+'부산서부_TimeTable.csv')
				modiReader = csv.reader(f2)
				for item in modiReader:
					if firstTemp == 0:
						firstTemp += 1
					else:
						row = [item[1],item[2],item[3],item[4],item[5],'부산서부(사상)',item[7],item[8],item[9],item[10]]
						copylist.append(row)
				...
				#resultDf.to_csv('./data/int_each/'+line[2] +'/'+'부산서부(사상)_TimeTable.csv',encoding='utf-8')
			print('./data/int_each/'+line[2] +'/'+ line[4]+'_TimeTable.csv')
			modifystr = './data/int_each/'+line[2] +'/'+ line[4]+'_TimeTable.csv'
